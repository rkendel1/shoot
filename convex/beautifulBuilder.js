...
exports.addFeature = exports.createBeautifulComponent = exports.refineUI = exports.buildCustomerFacingApp = void 0;
const server_1 = require("./_generated/server");
const values_1 = require("convex/values");
const api_1 = require("./_generated/api");
// Build beautiful, customer-ready frontend from natural language
exports.buildCustomerFacingApp = (0, server_1.action)({
    args: {
        specId: values_1.v.id("apiSpecs"),
        userDescription: values_1.v.string(), // "Build a beautiful pet adoption dashboard with search and filters"
...
API: ${spec.name}
Available Endpoints:
${spec.endpoints.map((e, i) => `${i + 1}. ${e.method} ${e.path} - ${e.summary || e.description || 'No description'}`).join('\n')}
...
Return as JSON:
{
  ...
  "deploymentReady": true
            ...
            const appId = await ctx.runMutation(api_1.api.apps.generateApp, {
                specId: args.specId,
                name: appName,
...
exports.refineUI = (0, server_1.action)({
    args: {
        appId: values_1.v.id("generatedApps"),
        refinementRequest: values_1.v.string(), // "Make it more colorful", "Add a search bar", "Make cards bigger"
    },
    handler: async (ctx, args) => {
...
Return as JSON:
{
  ...
  "visualDiff": "Description of visual changes user will see"
            ...
            // Save updates
            await ctx.runMutation(api_1.api.appUpdates.updateApp, {
                id: args.appId,
...
exports.createBeautifulComponent = (0, server_1.action)({
    args: {
        specId: values_1.v.id("apiSpecs"),
        componentDescription: values_1.v.string(), // "A product card with image, price, and add to cart button"
        endpoints: values_1.v.array(values_1.v.string()), // ["GET /products/{id}"]
    },
    handler: async (ctx, args) => {
        const spec = await ctx.runQuery(api_1.api.specs.getSpec, { id: args.specId });
        const openaiKey = process.env.OPENAI_API_KEY;
        ...
        try {
            const selectedEndpoints = spec.endpoints.filter((e) => args.endpoints.some(endpoint => `${e.method} ${e.path}` === endpoint));
            const prompt = `Create a BEAUTIFUL, production-ready React component.

Component Request: "${args.componentDescription}"

API Endpoints to use:
${selectedEndpoints.map((e) => `- ${e.method} ${e.path}: ${e.summary || e.description}`).join('\n')}
...
exports.addFeature = (0, server_1.action)({
    args: {
        appId: values_1.v.id("generatedApps"),
        featureDescription: values_1.v.string(), // "Add a shopping cart", "Add user authentication"
    },
    handler: async (ctx, args) => {
        const app = await ctx.runQuery(api_1.api.apps.getApp, { id: args.appId });
        const spec = await ctx.runQuery(api_1.api.specs.getSpec, { id: app.specId });
        const openaiKey = process.env.OPENAI_API_KEY;
        if (!openaiKey) {
            return { success: false, error: "OpenAI API key required" };
        }
