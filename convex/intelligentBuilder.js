...
exports.refineApp = exports.testWorkflow = exports.buildAppFromIntent = void 0;
const server_1 = require("./_generated/server");
const values_1 = require("convex/values");
const api_1 = require("./_generated/api");
// Intelligent app builder that selects endpoints and creates working functionality
exports.buildAppFromIntent = (0, server_1.action)({
    args: {
        specId: values_1.v.id("apiSpecs"),
        intent: values_1.v.string(), // Natural language: "I want to build a pet adoption system"
...
Selected Endpoints:
${analysis.selectedEndpoints.map((e) => `- ${e.endpoint}: ${e.purpose}`).join('\n')}
  ...
  "files": {
    ...
    "README.md": "complete docs"
            ...
            const appId = await ctx.runMutation(api_1.api.apps.generateApp, {
                specId: args.specId,
                name: appName,
                description: `${analysis.understanding}\n\nEndpoints used: ${analysis.selectedEndpoints.map((e) => e.endpoint).join(', ')}`,
                framework: "react",
            ...
            await ctx.runMutation(api_1.api.insights.saveWorkflow, {
                specId: args.specId,
...
exports.testWorkflow = (0, server_1.action)({
    args: {
        workflowId: values_1.v.id("workflows"),
        testData: values_1.v.string(), // JSON string with test inputs
    },
    handler: async (ctx, args) => {
        const workflow = await ctx.runQuery(api_1.api.insights.getWorkflows, {
            specId: "dummy" // We'll get it from the workflow
        });
        // Find the specific workflow
        const targetWorkflow = workflow.find((w) => w._id === args.workflowId);
        if (!targetWorkflow) {
...
exports.refineApp = (0, server_1.action)({
    args: {
        appId: values_1.v.id("generatedApps"),
        refinement: values_1.v.string(), // "Add validation" or "Make it faster"
    },
    handler: async (ctx, args) => {
...
${selectedEndpoints.map((e) => `- ${e.endpoint}: ${e.purpose}`).join('\n')}
...
Return as JSON with updated files:
{
  ...
  "explanation": "What was improved"
            ...
            return {
                ...
                message: `✅ Refined successfully!\n\n**Changes made:**\n${refinedCode.changes.map((c) => `- ${c}`).join('\n')}\n\n**Explanation:** ${refinedCode.explanation}`,
            };
        }
        catch (error) {
...
function generateBasicApp(spec, intent) {
    return {
        understanding: `Build functionality for: ${intent}`,
        selectedEndpoints: spec.endpoints.slice(0, 3).map((e) => ({
