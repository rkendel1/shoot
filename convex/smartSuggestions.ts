...
export const analyzeApiCapabilities = action({
  ...
  handler: async (ctx: ActionCtx, args) => {
    ...
    try {
      const endpointDetails = spec.endpoints.map((e: any) => ({
        method: e.method,
        path: e.path,
...
${endpointDetails.map((e: any) => `- ${e.method} ${e.path}: ${e.summary || 'No description'}`).join('\n')}
...
Endpoints:
${spec.endpoints.map((e: any) => `- ${e.method} ${e.path}: ${e.summary || ''}`).join('\n')}
...
export const suggestApiExtensions = action({
  args: {
    specId: v.id("apiSpecs"),
...
${spec.endpoints.map((e: any) => `- ${e.method} ${e.path}`).join('\n')}
...
export const generateRemix = action({
  args: {
    specId: v.id("apiSpecs"),
